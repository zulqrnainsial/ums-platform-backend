...
class CurriculumElectiveBulkService
{
    public function bulkAssign(array $data): array
    {
        ...
        if (!$user->can('subject.curriculum_elective_subject.create')) {
            abort(403, 'You are not allowed to assign elective subjects.');
        }
        ...
        return DB::transaction(function () use ($data, $tenantId, $user) {
            ...
            foreach ($items as $index => $item) {
                ...
                $keys = [
                    ...
                    'elective_group_code' => $data['elective_group_code'],
                    'subject_id' => $subject->id,
                ];

                $existing = CurriculumElectiveSubject::withTrashed()
                    ->where($keys)
                    ->first();

                if ($existing && !$overwriteExisting && !$existing->trashed()) {
                    $skipped++;
                    continue;
                }
                ...
                $payload = [
                    ...
                    'elective_group_name' => $data['elective_group_name'] ?? null,
                ...
                if ($existing) {
                    ...
                    CurriculumElectiveSubject::create(array_merge($keys, $payload, [
                        'created_by' => $user->id,
                    ]));
